...
| | |
|---|---|
| **Your role** | Scoring Simulator (D12) + QA + E2E Tests + Seed Data + CI/CD |
...
```
   ...
   Your role is high-leverage on FIVE fronts:

   1. SCORING SIMULATOR (D12)
      The differential piece of the demo. You build it end-to-end:
      the endpoint POST /scoring/simulate (wrapping Jesús' pure
      simulate() function), the admin UI screen D12, and the
      user guide for CMadrid. Antonio reviews the endpoint
      because it's legally sensitive.
   ...
   5. DEMO READINESS + KIOSK TORTURE (day 12)
   ...
   - Domain logic (Jesús) — but you DO consume his pure simulate()
     function from your endpoint
   - User-facing screens outside D12 (Alejandro) — but you DO build
     D12 with the base UI components Alejandro provides
   - Webfleet (Antonio)
```
...
Your code lives across these folders. The scoring simulator (D12) is yours end-to-end: backend endpoint, frontend screen, user guide, and E2E tests.

```
training/
├── apps/api/src/routes/
│   └── scoring.simulate.ts       ← YOUR CODE · endpoint POST /scoring/simulate
│                                    Jesús exposes packages/scoring/simulate()
│                                    pure; you write the HTTP wrapper +
│                                    ranking comparison + diff.
│                                    Antonio review obligatoria (legally
│                                    sensitive endpoint).
│
├── apps/web/src/pages/admin/
│   └── ScoringSimulator.tsx      ← YOUR CODE · screen D12
│                                    Alejandro hands you base components
...
└── docs/
    ...
    └── SIMULATOR-USER-GUIDE.md   ← YOUR DOC (for CMadrid, 1-2 pages,
                                     written in Spanish for the admin user)
```
...
DAY 10 (Thu 07/05) E2E Test #6: audit request + reevaluation flow.
                   START THE SIMULATOR (D12) — your differential piece:
                     Morning (4h): endpoint POST /scoring/simulate.
                       Jesús handed `packages/scoring/simulate()` pure
                       at end of day 9. You write the HTTP wrapper:
                       load closed attempts of the convocatoria, call
                       simulate(), build new ranking, compare with original,
                       identify candidates that cross the cut-off.
                       Open as PR — Antonio review obligatoria.
                     Afternoon (3h): screen D12 ScoringSimulator.tsx.
                       Alejandro hands you base components
                       (<ScoreBreakdown>, layouts) in the morning.
                       You build the override form + result table + diff.

DAY 11 (Fri 08/05) Polish the simulator end-to-end.
                     Morning: write docs/SIMULATOR-USER-GUIDE.md
                       (1-2 pages, Spanish, for the CMadrid admin).
                       Run E2E test #7 against your own screen — fix
                       any bug that appears.
                     Afternoon: E2E Test #8: full kiosk flow
                       (idle → RFID → active → close).
...
# 6. The simulator — your differential piece

The scoring simulator (D12) is yours end-to-end. It's the most visible piece of the demo: the admin types a rule change, hits "Simulate", and the ranking visibly reorders showing who crosses the cut-off. **You build it, you test it, you document it.** Antonio reviews the endpoint because it's legally sensitive (must not mutate closed attempts, must respect criteria_version pinned, must NOT issue PASS/FAIL per attempt — see invariants 3, 7, 9).

## 6.1 What you write

- **Endpoint `POST /scoring/simulate`** — HTTP wrapper around `packages/scoring/simulate()` (Jesús' pure function). Loads closed attempts of the convocation, calls simulate() with the override criteria, builds the new ranking, compares with the original, identifies candidates that cross the cut-off (in/out). Returns `{ranking_original, ranking_simulado, candidates_crossing_cutoff, summary }`. Antonio review obligatoria.
- **Screen D12 `ScoringSimulator.tsx`** — override form + result table + ranking diff. Alejandro provides the base UI components (`<ScoreBreakdown>`, layouts).
- **`docs/SIMULATOR-USER-GUIDE.md`** — 1-2 pages, Spanish (formal castellano), for the CMadrid admin user. Step-by-step: how to open D12, what an override means, how to read the ranking diff, what "crosses the cut-off" means in the competitive-exam model.
- **E2E test #7** — verifies that changing a threshold reorders the ranking visually and identifies in/out candidates. Runs against your own UI.
...
- The pure function `simulate()` in `packages/scoring/` — Jesús (delivered end of day 9).
- The base UI components consumed by D12 (`<ScoreBreakdown>`, page layout) — Alejandro.

## 6.3 Hard constraints (Antonio's review will reject violations)

- The simulator MUST NOT mutate any production data. Closed attempts keep their original score (invariant 3).
- The simulator MUST use the criteria_version pinned at attempt creation time when computing the original ranking, and apply overrides only as an in-memory transformation for the simulated branch (invariant 9).
- The endpoint MUST NOT emit a PASS/FAIL `decision` field per attempt — only scores and a simulated ranking (invariant 7, GDPR art. 22).
- The endpoint MUST be gated with `@require_role(['ADMIN'])` and audited (audit_log row per simulate call).

## 6.4 Timing

- Day 8–9: prepare seed data v4.
- Day 9 EOD: Jesús delivers `simulate()` pure. Confirm signature with him in chat.
- Day 10 morning: build the endpoint. Open PR. Tag Antonio for review.
- Day 10 afternoon: build screen D12 with components from Alejandro.
- Day 11 morning: write `SIMULATOR-USER-GUIDE.md`. Run E2E test #7 against your own screen. Fix any bug that surfaces.
- Day 11 18:00: internal demo — the simulator is the centerpiece.

If the simulator is unstable on Friday 08/05, you're the first to notice and the first to flag it (and the one fixing it).
...
- **Antonio reviews your `POST /scoring/simulate` endpoint** (and any code that touches scoring rules) — it's a legally sensitive endpoint. Open it as a PR with him as reviewer; expect comments on invariants 3, 7 and 9.
- Antonio speaks Spanish; the team operates in Spanish. **Team chat (`#training-equipo`) is bilingual — write in English when it helps clarity, especially in the first weeks.** DEMO-READINESS.md must be in Spanish (Antonio reads it as a status report). The user guide `SIMULATOR-USER-GUIDE.md` is also in Spanish (formal castellano — the CMadrid admin reads it).

## With Jesús

- **End of day 9: Jesús delivers `simulate()` pure** in `packages/scoring/`. You consume it from your endpoint on day 10. Confirm the function signature with him in chat before you start coding the wrapper.
- If he breaks an endpoint, he warns you in advance (it affects your E2E tests).
- If you need a new endpoint shape on the backend (anything beyond `simulate()`), you ask him.
